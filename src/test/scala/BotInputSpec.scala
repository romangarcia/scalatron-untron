...
class BotInputSpec extends FlatSpec {
  ...
  it should "parse React minibot command" in {
    ...
    assert( in === React(name="Pepe",time=100,view=new BotView("___W___W___W___"),energy=1000,master=Some(BotDirection(0, 0)),collision=Some(BotDirection(0,0)), generation=0) )
  }
  ...
  it should "parse React masterbot command" in {
    ...
    assert( in === React(name="Pepe",time=100,view=new BotView("___W___W___W___"),energy=1000,master=None,collision=Some(BotDirection(0,0)), generation=0) )
  }
}
